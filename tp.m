%%%%%sovling the Wasserstein distance for PPI
clear
clc
%load data
...
csArr=0.02:0.02:0.1;% the array of core-set size
   runNo=length(csArr);
    csSz=1;% the number index of core-sets
    %record result
...
emdTab_means=emdTab_means';
    ...
    if ~exist(['./Result/PPI/nonoise/test/%s',filefds(i).name()],'dir')
        path{1}=['./Result/PPI/nonoise/test/',filefds(i).name()];
        mkdir (path{1})
    end
    save(sprintf('./Result/PPI/nonoise/test/%s/kcenter_sink_size%.2f.mat',filefds(i).name(),tau),'emdTab_org','timTab_org','emdTab_cen','timTab_cen','emdTab_means',...
