%%%%%sovling the fractional Wasserstein distance for natural language
clear
clc
%% Bilinguistics
%load data
...
for i=3:Num
    ...
    tau=0:0.02:0.1;
    FM1=zeros(size(A,2),size(B,2));
    FM2=zeros(6,size(A,2),size(B,2));
    n1=size(A,2);
    n2=size(B,2);
    dim=size(A,1);
      runNo=6;
...
emdTab_means=emdTab_means';
    ...
    if ~exist(['./Result/BiL/noise/test/%s',filefds(i).name()],'dir')
        path{1}=['./Result/BiL/noise/test/',filefds(i).name()];
        mkdir (path{1})
    end
    save(sprintf('./Result/BiL/noise/test/%s/kcenter_sink_tau%.2f.mat',filefds(i).name(),csArr),'emdTab_org','timTab_org','emdTab_cen','timTab_cen','emdTab_means',...
