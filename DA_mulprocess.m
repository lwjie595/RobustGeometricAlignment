function DA_mulprocess()
%%%%%sovling the fractional Wasserstein distance for domain adaption
clear
clc
%load data
Num=4;
...
load(sprintf('Data/DA/extend/amazon_small/%s.mat',matname{i}));
   ...
   load(sprintf('Data/DA/extend/amazon_small/%s.mat',matname{Bmat}));
    B=fts';
    label2=labels;
    noise_index=setdiff([1,2,3,4],[i,Bmat]);
    load(sprintf('Data/DA/extend/amazon_small/%s.mat',matname{noise_index(1)}));
     C=fts';
     load(sprintf('Data/DA/extend/amazon_small/%s.mat',matname{noise_index(2)}));
     D=fts';
     dim=size(A,1);
    n1=size(A,2);
    n2=size(B,2);
...
tau=0:0.02:0.1;
    runNo=6;
csArr=0.1;% the array of core-set size
%record result
...
rA_thed=ceil(n1*csArr(outI));% the size of core-set of set A
rB_thed=ceil(n2*csArr(outI));% the size of core-set of set B
...
B=copB;
        ...
        [EMD1,FM_org]=Sinkhorn(TM*copA,copB,copWA,copWB,tau_f);
        timTab_org(outI,inI)=toc;
        emdTab_org(outI,inI)=EMD1;
        Error_org(outI,inI)=predict(TM,copA2,copB2,label1,label2,FM_org);
    ...
    disp('EMD based on Core-Set');
            ...
            Error_cen(outI,inI)=predict(TM2,copA2,copB2,label1,label2,FM);
            ...
            Error_kcenter(outI,inI)=predict(TM2,copA2,copB2,label1,label2,FM);
            ...
            Error_kmeans(outI,inI)=predict(TM2,copA2,copB2,label1,label2,FM);
            
            %StoasticOpt
                         tic
            [~,~,TM2]=SinkhornInit_batch(A,B,WA,WB,IT,tau_f,csArr);
            [EMD4,FM]=Sinkhorn(TM2*copA2,copB2,copWA2,copWB2,tau_f);
            ...
            Error_means(outI,inI)=predict(TM2,copA2,copB2,label1,label2,FM);
            ...
            Error_RC(outI,inI)=predict(TM2,copA2,copB2,label1,label2,FM);
...
emdTab_means=emdTab_means';
    ...
    timTab_kmedian= timTab_kmedian';  
        if ~exist(['./Result/DA/noise/test/%s',matname{i}],'dir')
            path{1}=['./Result/DA/noise/test/',matname{i}];
            mkdir (path{1})
        end
        save(sprintf('./Result/DA/noise/test/%s/%s_kcenter_sink_tau%.2f.mat',matname{i},matname{Bmat},csArr),'emdTab_org','timTab_org','emdTab_cen','timTab_cen','emdTab_means',...
