%%%%%sovling the fractional Wasserstein distance for PPI
clear
clc
%load data
filefds=dir('Data/PPI/emb1/*');
Num=numel(filefds);
for i=3:Num
load(['Data/PPI/emb1/',filefds(i).name(),'/A.mat']);
load(['Data/PPI/emb1/',filefds(i).name(),'/B.mat']);
n1=size(A,2);
n2=size(B,2);
WA=ones(1,n1);
WB=ones(1,n2);
...
rA_thed=ceil(size(A,2)*csArr(1));% the size of core-set of set A
rB_thed=ceil(size(B,2)*csArr(1));% the size of core-set of set B
%
...
%     % EMD based on Core-Set
    disp('EMD based on Core-Set');
         ...
         tic
            ...
            [EMD4,~]=Sinkhorn(TM2*copA2,copB2,copWA2,copWB2,tau_f2);
...
emdTab_means=emdTab_means';
    ...
    if ~exist(['./Result/PPI/noise/test/%s',filefds(i).name()],'dir')
        path{1}=['./Result/PPI/noise/test/',filefds(i).name()];
        mkdir (path{1})
    end
    save(sprintf('./Result/PPI/noise/test/%s/kcenter_sink_tau%.2f.mat',filefds(i).name(),csArr),'emdTab_org','timTab_org','emdTab_cen','timTab_cen','emdTab_means',...
